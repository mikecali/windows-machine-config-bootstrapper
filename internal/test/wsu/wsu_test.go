...
func testEastWestNetworking(t *testing.T, node *v1.Node, vm e2ef.WindowsVM) {
	...
	winServerDeployment, err := deployWindowsWebServer(
		"win-webserver-"+vm.GetCredentials().GetInstanceId(), vm, affinity)
...
func testNorthSouthNetworking(t *testing.T, node *v1.Node, vm e2ef.WindowsVM) {
	...
	loadBalancer, err := createLoadBalancer("win-webserver-"+vm.GetCredentials().GetInstanceId(),
		*winServerDeployment.Spec.Selector)
